...
BRRO_sites2 <- suppressMessages(read_csv('data/BRROdata_sites2013_Nov2019.csv')) # made in AmbientNetworkRebuild2019.Rmd
  ...
  observe({
    leafletProxy( ns("map"), data = userDataset()) %>%
      ...
      addCircleMarkers(data=userDataset(),~Longitude,~Latitude,radius=5,color=~pal(FDT_SPG_CODE),#fillColor="blue",
                       ...
                       popup=leafpop::popupTable(userDataset(),zcol=c("FDT_STA_ID","Huc6_Vahu6","FDT_SPG_CODE","nObservations","yearsSampled"))) %>%
      addLayersControl(baseGroups=c('Open Street Map','Esri World Imagery','Stamen Terrain Background'),
...
watershedMapUI <- function(id){
  ...
  tagList(
    ...
    column(3, 
           wellPanel(
             selectInput(ns('polygonChoices'), 'Watershed Choices', 
                         choices = c('All BRRO Watersheds','2022IR Needs','2024IR Needs',
    ...
    helpText("    - Watersheds that need additional samples for 2022 IR Window (2022IR Needs)"),
    helpText("    - Watersheds that need additional samples for 2024 IR Window (2024IR Needs)"),
    helpText("    - Watersheds we may be able to stop sampling (Not On Mary's List). These are 
  ...
  observe({
    leafletProxy( ns("QAmap"), data = userWatershedDataset) %>%
      ...
      addPolygons(data=userWatershedDataset,color=~pal2(get(colName())),
                  layerId=~userWatershedDataset@data$VAHUC6,
                  fill=0.1,stroke=0.2,group="Watershed Query",
                  popup=leafpop::popupTable(userWatershedDataset@data,
                                   zcol=c('VAHU6', 'VaName', 'VAHU5'))) %>%
